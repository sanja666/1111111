...
function renderDiscountList() {
  ...
  if (!list) return;
  list.innerHTML = '';
  const discounts = state.discounts.slice().reverse();
  const hasBlackPass = Boolean(state.activePass && state.activePass.name.includes('BLACK TIER'));

  if (state.blackTrialActive) {
    const li = document.createElement('li');
    li.innerHTML = '<strong>BLACK TIER trial 24h</strong><span>Aktīvs — iekšējais pulkstenis tikšķ, izmanto ēnu privilēģijas gudri.</span>';
    list.appendChild(li);
  }

  if (hasBlackPass) {
    const li = document.createElement('li');
    const validUntil = new Date(state.activePass.validUntil).toLocaleDateString();
    li.innerHTML = `<strong>Ēnas līmeņa abonements</strong><span>Derīgs līdz ${validUntil}. Kontrolieri skatās citādi.</span>`;
    list.appendChild(li);
  }

  discounts.forEach(discount => {
    const li = document.createElement('li');
    const created = discount.createdAt ? new Date(discount.createdAt).toLocaleString() : 'tikko reģistrēts';
    li.innerHTML = `<strong>${discount.type}</strong><span>Reģistrēts: ${created}</span>`;
    list.appendChild(li);
  });

  if (!list.children.length) {
    list.innerHTML = '<li>Nav reģistrētu kuponu. Gods Wheel gaida tavu drosmi.</li>';
  }
}
