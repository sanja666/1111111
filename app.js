...
function addDiscount(discount) {
  const normalized = {
    ...discount,
    type: discount.type || discount.label || 'Nezināma privilēģija',
    label: discount.label || discount.type || 'Nezināma privilēģija',
    createdAt: discount.createdAt || Date.now()
  };
  const discounts = [...state.discounts, normalized];
...
function applyDiscountToPlan(plan) {
  let finalPrice = plan.price;
  let usedIndex = null;
  let note = '';
  state.discounts.forEach((discount, index) => {
    if (usedIndex !== null) return;
    const type = (discount.type || '').toLowerCase();
    if (type.includes('bezmaksas')) {
      finalPrice = 0;
      usedIndex = index;
      note = 'Aktivizēts “Bezmaksas mēnesis?” kupons. Sistēma cer, ka tas nebija kļūda.';
    } else if (type.includes('-10')) {
      finalPrice = +(plan.price * 0.9).toFixed(2);
      usedIndex = index;
      note = 'Piemērota -10% atlaižu privilēģija no ēnu arhīva.';
    }
  });
  return { finalPrice, usedIndex, note };
}
...
function handlePlanPurchase(planId) {
  ...
  const { finalPrice, usedIndex, note } = applyDiscountToPlan(plan);
  ...
  if (usedIndex !== null) {
    ...
    discounts.splice(usedIndex, 1);
    localStorage.setItem(STORAGE_KEYS.discounts, JSON.stringify(discounts));
  }

  adjustBalance(-finalPrice);
  ...
  const messages = ['Pirkums veiksmīgs ✅'];
  if (finalPrice !== plan.price) {
    messages.push(`Nomaksāts: ${finalPrice.toFixed(2)} € (oriģināli ${plan.price.toFixed(2)} €).`);
  }
  if (note) {
    messages.push(note);
  }
  if (finalPrice === 0) {
    messages.push('Šo mēnesi sistēma izliekas, ka viss ir kārtībā.');
  }
  alert(messages.join('\n'));
  loadState();
  renderAll();
...
function renderControllerActivity() {
  ...
  let level = 'zema';
  if (reportCount > 8) level = 'BRĪDINI MAMMU';
  else if (reportCount > 5) level = 'augsta vigil';
  else if (reportCount > 2) level = 'vidēja';
...
function renderDiscountList() {
  ...
  if (!state.discounts.length) {
    list.innerHTML = '<li>Nav aktīvu kuponu. Gods Wheel gaida tavu griezienu.</li>';
    return;
  }
  state.discounts
    .map((discount, index) => ({ discount, index }))
    .reverse()
    .forEach(({ discount, index: originalIndex }) => {
      const li = document.createElement('li');
      const date = new Date(discount.createdAt);
      const label = discount.label || discount.type;
      li.innerHTML = `<strong>${label}</strong> — piešķirts ${date.toLocaleString()}`;
      const btn = document.createElement('button');
      btn.className = 'btn ghost mini';
      btn.textContent = 'Atzīmēt izmantotu';
      btn.addEventListener('click', () => consumeDiscount(originalIndex));
      li.appendChild(btn);
      list.appendChild(li);
    });
}

function consumeDiscount(index) {
  const discounts = [...state.discounts];
...
function renderShadowIntel() {
  const list = document.getElementById('shadow-intel-list');
  if (!list) return;
  const activePlan = state.activePass ? `${state.activePass.name} (${state.activePass.id})` : 'Nav aktīvas biļetes';
  const discountsCount = state.discounts.length;
  const lastPrize = state.lastPrize || 'nav vēl griezts';
  const lastController = state.controllerReports[state.controllerReports.length - 1];
  const controllerText = lastController
    ? `${lastController.where} (${lastController.time}, risks ${lastController.danger}/5)`
    : 'Nav svaigu ziņojumu, kas pats par sevi ir aizdomīgi.';
  const incidents = state.ticketsHistory.length;
  const shadowStatus = state.blackTrialActive || (state.activePass && state.activePass.name.includes('BLACK TIER'))
    ? 'ĒNAS STATUSS AKTĪVS — sveiciens no pagrīdes.'
    : 'Ēnu statuss gaida iniciāciju.';
  list.innerHTML = `
    <li><span>Operatīvais statuss:</span> <strong>${shadowStatus}</strong></li>
    <li><span>Lojalitātes plāns:</span> <strong>${activePlan}</strong></li>
    <li><span>Kuponu krājums:</span> <strong>${discountsCount}</strong></li>
    <li><span>Gods Wheel trofeja:</span> <strong>${lastPrize}</strong></li>
    <li><span>Pēdējais kontrolieru novērojums:</span> <strong>${controllerText}</strong></li>
    <li><span>Incidentu reģistrs:</span> <strong>${incidents} lietas</strong></li>
  `;
}
